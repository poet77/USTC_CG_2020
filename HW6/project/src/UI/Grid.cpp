...
void Ubpa::Grid::AddTextBox(const std::string& title, const std::function<void(std::string)> slot)
{
	auto textBox = new QLineEdit;
	page->connect(textBox, &QLineEdit::returnPressed, [textBox, slot] {
		slot(textBox->text().toStdString());
		});
	AddRow(title, textBox);
}
